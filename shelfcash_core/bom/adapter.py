...
from shelfcash_core.bom.contracts import RecipeRecord, UnitConversionRule
...
def _parse_optional_rate(value: object) -> float | None:
    if pd.isna(value) or (isinstance(value, str) and not value.strip()):
        return 0.0
    return _parse_number(value)


def _raise_product_unit_conflicts(
...
def adapt_recipes(recipes: pd.DataFrame) -> list[RecipeRecord]:
    ...
    for row_number, row in frame.reset_index(drop=True).iterrows():
        ...
        if (
            ...
            row_errors.append("effective_to must be >= effective_from")
...
def _validate_recipe_metadata_consistency(records: list[RecipeRecord]) -> None:
