...
from shelfcash_forecast.contracts import (
    ForecastPackage,
    ForecastPrediction,
)
...
from shelfcash_forecast.exceptions import FeatureSchemaError, InsufficientDataError
...
def predict_demand(
    ...
    if forecast_horizon < 1 or forecast_horizon > max(config.horizons):
        raise ValueError(
            f"forecast_horizon phải nằm trong 1..{max(config.horizons)}."
        )
    ...
    if missing_features:
        raise FeatureSchemaError(
            f"Runtime thiếu features: {sorted(missing_features)}"
        )
    ...
    global_warnings = list(quality_report.warnings)
