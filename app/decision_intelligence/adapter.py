...
class ShelfCashDecisionIntelligenceAdapter:
    ...
    def _evidence(self, brief: DecisionBriefFacts):
        ...
        for index, demand in enumerate(brief.ingredient_demand):
            collector.add(layer="M3", evidence_type="ingredient_demand", source_object="DecisionRun.package_json", source_path=f"package.ingredient_demand[{index}]", semantics="quantile", entities={"store_id": brief.store_id, "ingredient_id": demand.ingredient_id}, payload={"p25": demand.p25, "p50": demand.p50, "p75": demand.p75, "unit": demand.unit}, text=f"Persisted ingredient demand for {demand.ingredient_name or demand.ingredient_id}: P50={demand.p50} {demand.unit or ''}.")
