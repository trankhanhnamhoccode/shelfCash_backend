...
class DecisionPlanningService:
 ...
 def generate_decision(self,store,body,key=None):
  ...
  with self.factory() as s:
   ...
   package=self._decision_package(rid,store,forecast,mode,result,request,baseline,demands,scenario_metadata,forecast_trace)
 ...
 @staticmethod
 def _decision_package(rid,store,forecast,mode,result,request,baseline,demands,scenario_metadata=None,forecast_trace=None):
  def metric(sim):
