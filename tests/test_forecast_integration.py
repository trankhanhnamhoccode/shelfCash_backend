...
def test_db_to_real_core_to_persisted_predictions(client, monkeypatch):
    ...
    with sf() as s:
        ...
        s.add(SupplierIngredientTermModel(constraint_id="tea-term",store_id="STORE_001",supplier_id="tea-supplier",ingredient_id="tea-leaf",unit_cost=100,moq=Decimal("1"),pack_size=Decimal("0.5"),lead_time_days=0,safety_stock=Decimal("0.5"),capacity=Decimal("100"),unit="kg",version=1,active=True,source="test"))
    ...
    assert demand_get.json()==demand_replay.json() and plans_get.json()==plans_created.json()
